...
from yuri.config import EyesConfig
...
def set_angle(servo: Servo, target_angle: Optional[float] = None):
    if target_angle is None:
        target_angle = servo.angle or 0

    servo.angle = int(max(min(target_angle, servo.actuation_range), 0))
...
async def move(
    ...
    while not math.isclose(servo.angle or 0, target_angle, rel_tol=0.02):
        last_angle = servo.angle or 0
        ...
        if (
            math.isclose(servo.angle or 0, last_angle, rel_tol=0.02)
...
class Eyes:
    ...
    async def blink_loop(self):
        while True:
            ...
            await self.open()
            await asyncio.sleep(random.random() * 6.0)
    ...
    async def vert_look(self, offset: float):
        new_left = (self.left_y.angle or 0) + offset
        new_right = (self.right_y.angle or 0) - offset
    ...
    async def horiz_look(self, offset: float):
        await asyncio.gather(
            ...
            move(
                self.left_x,
                (self.left_x.angle or 0) + offset,
                smoothing_factor=self.config.left_eye.movement_smoothing,
            ),
            move(
                self.right_x,
                (self.right_x.angle or 0) + offset,
                smoothing_factor=self.config.right_eye.movement_smoothing,
            ),
        )
